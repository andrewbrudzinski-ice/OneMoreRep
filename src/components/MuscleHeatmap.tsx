 ...
 * working volume — dark (untrained) → electric lime (worked hard), so a glance
 * answers "am I neglecting X?". Tapping a region selects it (syncs with the
 * ranked list below). Not anatomically precise, just recognizable.
 */
...
export function MuscleHeatmap({
  ...
  // Cold (untrained) fill = surface 2 (#1B2027), read from the theme.
  const coldRGB = readChannels('--s-800', [27, 32, 39]);
  ...
  const region = (id: string, children: React.ReactNode) => {
    ...
    return (
      <g
        ...
        className={isSel(id) ? 'stroke-ink' : 'stroke-ground'}
  ...
  return (
    <div className="grid grid-cols-2 gap-2">
      <Silhouette label="Front">
        {/* head (outline only) */}
        <circle cx={50} cy={16} r={9} fill="#1B2027" stroke="#0D1014" strokeWidth={0.75} />
        {region(
          'mg-shoulders',
          <>
